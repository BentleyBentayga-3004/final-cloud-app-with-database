...
def submit(request, course_id):
    ...
    # Lấy enrollment object tương ứng của user với khóa học
    enrollment = Enrollment.objects.filter(user=user, course=course).first()
    
    if request.method == 'POST':
        # Tạo một submission mới liên kết với enrollment
        submission = Submission.objects.create(enrollment=enrollment)
        
        # Thu thập các lựa chọn từ form
        submitted_answers = extract_answers(request)
        for choice_id in submitted_answers:
            choice = Choice.objects.get(pk=choice_id)
            submission.choices.add(choice)
            
        # Chuyển hướng đến trang xem kết quả thi
        return HttpResponseRedirect(reverse('onlinecourse:show_exam_result', args=(course.id, submission.id)))
    
    return redirect(reverse('onlinecourse:course_details', args=(course.id,)))
...
def show_exam_result(request, course_id, submission_id):
    ...
    score = 0
    # Tính điểm và kiểm tra kết quả cho từng câu hỏi
    for question in course.question_set.all():
        ...
        # Nếu học viên chọn đúng toàn bộ đáp án của câu hỏi
